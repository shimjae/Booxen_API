...
public class OrderDAO {
	  private SqlSessionFactory sqlSessionFactory = null;
	   ...
	   public  List  GetResetList(java.util.HashMap map)throws SQLException{
	    	List list = null;
	        
	        SqlSession session = sqlSessionFactory.openSession();
	 
	        try {
	        	list = session.selectList("Goods.GetResetList",map);
	        } finally {
	            session.close();
	        }
	   ...
	   public void UptOrder(HashMap map){
		   	 
	        SqlSession session = sqlSessionFactory.openSession();
	 
	        try {
	            session.update("Goods.UptOrder", map);
	        } finally {
	            session.commit();
	            session.close();
	        }
