...
public class GoodsDAO {
	  ...
	  private SqlSessionFactory sqlSessionFactory = null;
	    ...
	    public int DelRegItem(java.util.HashMap map){
	        int id = -1;
	        SqlSession session = sqlSessionFactory.openSession();
	 
	        try {
	            id = session.delete("Goods.DelRegItem", map);
	        } finally {
	            session.commit();
	            session.close();
	        }
	    ...
	    public int DelRegItem_Dupple(java.util.HashMap map){
	        int id = -1;
	        SqlSession session = sqlSessionFactory.openSession();
	 
	        try {
	            id = session.delete("Goods.DelRegItem_Dupple", map);
	        } finally {
	            session.commit();
	            session.close();
	        }
